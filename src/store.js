...
class JsonStore {
  ...
  createTask(payload, actorId) {
    const task = {
      ...
      minutesPerOccurrence: Number(payload.minutesPerOccurrence || 0),
  ...
  updateTask(id, patch, actorId) {
    ...
    const allowed = [
      ...
      'periodicity', 'frequencyPerMonth', 'minutesPerOccurrence', 'monthlyHours',
      'priority', 'difficulty', 'status', 'dueDate', 'wellbeingSignal', 'tags'
    ];
    for (const key of allowed) {
      if (Object.prototype.hasOwnProperty.call(patch, key)) {
        task[key] = ['estimatedHours', 'realHours', 'frequencyPerMonth', 'minutesPerOccurrence', 'monthlyHours'].includes(key) ? Number(patch[key]) : patch[key];
      }
    }
