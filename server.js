...
const { computeAnalytics, toCsvReport } = require('./src/analytics');
...
function normalizeTaskPayload(body) {
  ...
  if (payload.estimatedHours !== undefined) {
    ...
    if (!Number.isFinite(payload.estimatedHours) || payload.estimatedHours <= 0 || payload.estimatedHours > 200) {
      const error = new Error('El tiempo estimado debe ser un numero entre 0 y 200.');
      error.statusCode = 400;
      throw error;
    }
  }
...
function htmlReport(analytics) {
  const rows = analytics.users.map((user) => `
    <tr>
      ...
      <td>${user.capacityHoursPerWeek}</td>
      <td>${user.workloadScore}</td>
      <td>${user.utilizationPercent}%</td>
...
<style>
body{font-family:Arial,sans-serif;margin:32px;color:#1f2937}h1{margin-bottom:0}.muted{color:#6b7280}table{border-collapse:collapse;width:100%;margin-top:24px}th,td{border:1px solid #d1d5db;padding:8px;text-align:left}th{background:#f3f4f6}.cards{display:flex;gap:12px;flex-wrap:wrap}.card{border:1px solid #d1d5db;border-radius:12px;padding:12px;min-width:160px}.print{margin:16px 0}@media print{.print{display:none}}
</style>
</head>
<body>
<h1>ASAP design by Jeisson Steven Herrera Baquero</h1>
<p class="muted">Reporte de cargas laborales generado el ${escapeHtml(analytics.generatedAt)}. Use Ctrl/Cmd + P para guardar como PDF.</p>
<button class="print" onclick="window.print()">Imprimir / guardar PDF</button>
<div class="cards">
  <div class="card"><strong>Carga total</strong><br>${analytics.summary.totalWorkload}</div>
  <div class="card"><strong>Uso promedio</strong><br>${analytics.summary.averageUtilizationPercent}%</div>
  <div class="card"><strong>Tareas vencidas</strong><br>${analytics.summary.overdueTasks}</div>
  <div class="card"><strong>Usuarios en riesgo</strong><br>${analytics.summary.overloadedUsers}</div>
</div>
<table>
<thead><tr><th>Usuario</th><th>Rol</th><th>Capacidad</th><th>Carga</th><th>Utilizacion</th><th>Abiertas</th><th>Vencidas</th><th>Riesgo</th></tr></thead>
<tbody>${rows}</tbody>
</table>
</body></html>`;
}
...
async function handleApi(req, res, url) {
  ...
  if (method === 'POST' && pathname === '/api/tasks') {
    ...
    requireFields(body, ['title', 'estimatedHours', 'priority', 'difficulty', 'dueDate']);
